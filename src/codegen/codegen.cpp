    ...
    switch (instr.opcode) {
        ...
        case IROpcode::CALL: {
            ...
            if (func_name == "print") {
                ...
                std::string potential_label = "func_" + instr.src1.value;
                if (function_registry.find(potential_label) != function_registry.end()) {
                    // This is a user-defined function call
                    std::string result = sanitizeName(instr.dest.value);
                    std::string call_expr = sanitizeName(potential_label) + "(";
                    
                    // Add parameters
                    for (size_t i = 0; i < pending_params.size(); i++) {
                        if (i > 0) call_expr += ", ";
                        call_expr += sanitizeName(pending_params[i]);
                    }
                    call_expr += ")";
                    ...
                    if (instr.dest.type != IROperandType::NONE) {
                        ...
                        emitLine("int64_t " + result + " = " + call_expr + ";");
                        variable_types[instr.dest.value] = "int64_t";
                    } else {
...
// Helper: Check if a label is a function label
bool CCodeGenerator::isFunctionLabel(const std::string& label) const {
    return label.find("func_") == 0;
}

// Helper: Check if a function name is a built-in
bool CCodeGenerator::isBuiltInFunction(const std::string& name) const {
    static const std::unordered_set<std::string> builtins = {
        "print", "len", "str", "int", "float", "abs", "type", "input", "range"
    };
    return builtins.find(name) != builtins.end();
}

// First pass: Collect all function definitions
void CCodeGenerator::collectFunctions(const std::vector<BasicBlock>& blocks) {
    function_registry.clear();
    ...
    for (size_t i = 0; i < blocks.size(); i++) {
        ...
        if (isFunctionLabel(block.label)) {
            ...
            func_info.label = block.label;
            func_info.name = block.label.substr(5);  // Remove "func_" prefix
            func_info.has_return_value = false;
            
            // Collect parameters from DECLARE instructions at the start
            for (const auto& instr : block.instructions) {
                if (instr.opcode == IROpcode::DECLARE) {
                    func_info.parameters.push_back(instr.dest.value);
                } else if (instr.opcode != IROpcode::NOP) {
                    // Stop at first non-DECLARE instruction
                    break;
                }
            }
            ...
            for (const auto& instr : block.instructions) {
                if (instr.opcode == IROpcode::RETURN && instr.dest.type != IROperandType::NONE) {
                    func_info.has_return_value = true;
                    break;
                }
            }
            
            function_registry[func_info.label] = func_info;
        }
    }
}

// Generate forward declarations for all user-defined functions
void CCodeGenerator::generateFunctionForwardDeclarations() {
    if (function_registry.empty()) {
        return;
    }
    
    output << "// Forward declarations\n";
    for (const auto& [label, func_info] : function_registry) {
        // Return type
        output << (func_info.has_return_value ? "int64_t" : "void");
        output << " " << sanitizeName(func_info.label) << "(";
        
        // Parameters
        for (size_t i = 0; i < func_info.parameters.size(); i++) {
            if (i > 0) output << ", ";
            output << "int64_t " << sanitizeName(func_info.parameters[i]);
        }
...
void CCodeGenerator::generateFunctionDefinition(const std::string& label, const BasicBlock& block,
                                                const std::vector<BasicBlock>& all_blocks, size_t& block_idx) {
    auto it = function_registry.find(label);
    ...
    // Function signature
    functions_code << (func_info.has_return_value ? "int64_t" : "void");
    functions_code << " " << sanitizeName(label) << "(";
    
    // Parameters
    for (size_t i = 0; i < func_info.parameters.size(); i++) {
        if (i > 0) functions_code << ", ";
        functions_code << "int64_t " << sanitizeName(func_info.parameters[i]);
    }
    
    functions_code << ") {\n";
    
    // Function body - temporarily redirect output
    std::stringstream original_output;
    ...
    // Append function body to functions_code BEFORE closing brace
    functions_code << output.str();
    functions_code << "}\n\n";
    
    // Restore main output
    output.str("");
    output.clear();
    output << original_output.str();
